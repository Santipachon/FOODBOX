# 📝 Intake del cliente — Insumos para arrancar

> Este documento se **rellena** con la información que nos pasa el cliente (Santi).
> Cada vez que llegue un dato, lo escribimos aquí y hacemos commit. Así queda el
> historial de qué sabíamos y cuándo.
>
> Estado de cada bloque: ⬜ pendiente · 🟡 parcial · ✅ completo

---

## A) La carta actual — ✅ (recibida y estructurada)

- **Imágenes de la carta actual:** ✅ 10 piezas en `assets/carta-actual/`.
- **Categorías, productos, descripciones y precios:** ✅ 18 hamburguesas + adición Angus,
  transcritas en `docs/carta/carta-fuente.md` y modeladas en `data/menu.json`.
- **Combos/adiciones/variaciones:** ✅ Combo = personal **+$9.500** (hamburguesa+papas+bebida),
  consistente en todas. Adición: cambio/porción de Certified Angus Beef.
- **Pendiente de confirmar:** vigencia de precios y disponibilidad actual de cada producto.

## B) Identidad de marca — 🟡 (casi completa)

> Estructurada en `docs/marca/identidad-de-marca.md`. Fuentes en `assets/marca/`.

- **Nombre exacto del restaurante:** ✅ **FOODBOX Burger** (formato express: FOODBOX Express).
- **Ciudad:** ✅ Yopal, Casanare 🇨🇴.
- **Logo:** 🟡 Tenemos capturas (`assets/marca/`). Falta el original en **SVG/PNG transparente**.
- **Colores de marca (HEX):** 🟡 Definidos por extracción (amarillo `#F7B500`, negro `#0F0F0F`,
  azul `#00A8E8`). Falta confirmar códigos oficiales si existen.
- **Tipografía / estilo buscado:** ✅ Street / urbana con impacto. Sugerencias web definidas.
  Falta confirmar las fuentes reales del logo.
- **Personalidad de marca:** ✅ Callejera, energética, orgullo local, "Meropower ⚡",
  calidad de competencia (Burger Master 2022–2026).
- **Redes sociales:** ✅ Instagram [@foodboxburger](https://instagram.com/foodboxburger) (~29.6k).
- **Sedes:** ✅ Principal (Cra 40 # 33 esq.) y Alcaraván (C.C. Alcaraván, formato Express).

## C) Fotos — 🟡 (estrategia definida, procesamiento pendiente)

- **Fotos disponibles y calidad:** ✅ Las de la carta actual (buena calidad, pero con 1–3
  hamburguesas por imagen y texto encima).
- **Cuáles sirven vs. cuáles regenerar:** ✅ Plan en `docs/carta/estrategia-imagenes.md`:
  se recorta 1 hamburguesa por imagen, se limpia con GPT Image (preservando el producto real)
  y se exporta a **WebP transparente 1:1**. Se procesan las 18 para uniformidad.

## D) Experiencia y pedido — ⬜

- **¿Cómo piden hoy los clientes?** _(WhatsApp / mesa / domicilio / mostrador)_
- **Fase 1: ¿solo informativa o con botón "Pedir por WhatsApp"?** _(pendiente)_
- **¿Manejan domicilios? Zonas / costo de envío:** _(pendiente)_

## E) Administración y técnica — ⬜

- **¿Quién edita la carta y qué tan cómodo se siente con la tecnología?** _(pendiente)_
- **¿Dominio / hosting propio? Presupuesto aprox.:** _(pendiente)_
- **¿Preferencia de tecnología o a criterio nuestro?** _(pendiente)_

---

### 📌 Preguntas abiertas / por confirmar

1. ✅ ~~Nombre~~ → **FOODBOX Burger**, Yopal.
2. ¿Moneda? (asumo COP — pesos colombianos — por defecto).
3. Nivel de autonomía real que quiere el dueño → define el stack (ver `docs/02-arquitectura.md`).
4. ¿La carta digital es **una sola para ambas sedes** o una por sede? (afecta horarios/productos).
5. Logo original en **SVG/PNG transparente** en alta resolución.
6. WhatsApp/teléfono de pedidos para la carta.
